...
### Using arsenal
```{r}
table_three <- tableby(~ weight_1 + child_ses + currnt_ses + hrs_no_eat_1 + hunglvl + delicious_taste + calories_eaten, data = cleans2)
...
my_labels <- list(
  ...
  hrs_no_eat = "Hours since eating",
  hunglvl = "Hunger (1-7)",
  delicious_taste = "Liking of the cookies (1-7)",
  calories_eaten = "Total calories consumed"
)

table_four <- tableby (~ weight_1 + child_ses + currnt_ses + hrs_no_eat_1 + hunglvl + delicious_taste + calories_eaten, data = cleans2)
...
```{r}
library(naniar)
>>>>>>> 692967ef31f7e5a6d240811dcd15e3fb5d1a320a
