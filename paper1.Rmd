...
```{r}
cleans2_filter <- cleans2 %>% filter (dummy1_water_sprite == "1")
View(cleans2_filter)
```

```{r}
table_two <- tableby( ~ weight_1 + child_ses + currnt_ses + hrs_no_eat_1 + hunglvl + delicious_taste + calories_eaten, data = cleans2_filter)
...
```{r}
cleans2_filter2 <- cleans2 %>% filter (dummy2_sprite_water == "1")
View(cleans2_filter2)
```

```{r}
table_two2 <- tableby( ~ weight_1 + child_ses + currnt_ses + hrs_no_eat_1 + hunglvl + delicious_taste + calories_eaten, data = cleans2_filter2)
...
summary(table_two2, labelTranslations = my_labels,
        title = "Table 2. Descriptive Statistics for Study 2")
```
...
### Using arsenal
```{r}
table_three <- tableby(sprite_water_dummy2 ~ weight + child_ses + adult_ses + blood_draw_2_post_maip + delicious_taste + calories_eaten, data = cleans3)
